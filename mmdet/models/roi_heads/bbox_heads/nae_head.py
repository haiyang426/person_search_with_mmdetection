...
class NaeHead(BaseModule):
    ...
    def forward(self, feat_p_before: Tuple[Tensor], feat_p_after: Tuple[Tensor]) -> tuple:
        ...
        # x_reid = self.fc_reid(x_reid.view(x_reid.size(0), -1))
        # feat_p_before = F.adaptive_max_pool2d(feat_p_before, 1).flatten(1)
    ...
    def loss(self,
        ...
        if id_pred is not None:
            losses['loss_id'] = self.loss_id(id_pred[ids_weights>0], ids[ids_weights>0])
            
            # cluster_id_labels = self.loss_id.get_label(ids[ids_weights>0])
            # new_id_labels = ids.clone()
            # new_id_labels[ids_weights > 0] = cluster_id_labels
            
            # losses['loss_triplet'] = self.loss_triplet(id_pred, new_id_labels)
        return losses
